...
prep_model_syntax <- function(pgs, constraints="unconstrained",mplusdata){
   ...
   mplusdata_tmp <- mplusdata %>% 
      select(indid, m_id, sex, parity, matches("mdrink|int|ext"),matches(pgs)  ) %>% 
   ...
   mplusdata_tmp <- mplusdata_tmp %>% 
      mutate(lpgs_c = log(pgs_c+abs(min(mplusdata_tmp$pgs_c,na.rm=T)-1)))
   ...
   df_lists <- mplusdata_tmp %>% 
      select(mdrink_1, mdrink_2, mdrink_3,mdrink_pre,
             pgs_m, pgs_f,matches("pgs_c"),sex,parity) %>% 
      summarise_all(list) %>% 
   ...
   prepareMplusData(mplusdata_tmp_w_ints, filename = paste0("./data/",pgs,"_mplus.dat"),
                    inpfile = TRUE)
   
   # Read in the resulting input file, and use it as the basis of input files for
   # each model, with each outcome 
   
   stem <-readLines( paste0("./data/",pgs,"_mplus.inp"))
   ...
   for(out in outcomes){
      ...
      filecon <- paste0("./scripts/mplus/", pgs, "_", out, "_",constraints,".inp")
...
MODEL:
   ...
   i_m_2 BY ",out,"_2 (g1_2);
   i_m_3 BY ",out,"_3 (g1_3);
